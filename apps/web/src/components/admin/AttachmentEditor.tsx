...
const LABEL: Record<Role, { title: string; hint: string; add: string; accept: string }> = {
  POSTER: {
    ...
    hint: "ใช้เป็นภาพพื้นหลัง/ภาพเปิดของรายการนี้ — มีได้รูปเดียว ใส่ใหม่จะแทนที่ของเดิม (ไม่เกิน 5 MB)",
    add: "อัปโหลดรูปหลัก",
    accept: IMAGE_ACCEPT,
  },
  BROCHURE: {
    title: "หน้าโบรชัว",
    hint: "รูปทั้งหน้าเรียงต่อกัน — ใช้แทนเนื้อหาแบบข้อความ ใส่ได้หลายหน้า จัดลำดับได้ (ไฟล์ละไม่เกิน 15 MB)",
    add: "+ อัปโหลดหน้าโบรชัว",
    accept: IMAGE_ACCEPT,
  },
  GALLERY: {
    title: "แกลเลอรีรูป",
    hint: "ใส่ได้หลายรูป จัดลำดับได้ — แสดงเป็นแกลเลอรีในหน้าเว็บ (ไฟล์ละไม่เกิน 5 MB)",
    add: "+ อัปโหลดรูป",
    accept: IMAGE_ACCEPT,
  },
  DOWNLOAD: {
